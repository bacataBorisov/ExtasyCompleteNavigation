...
// VMGCalculator.swift - new refactored version using DiagramLoader
import Foundation
...
class VMGCalculator {
    ...
    func evaluateDiagram(windForce: Double, windAngle: Double) -> Double {
        ...
        guard let j = wind.firstIndex(where: { $0 > windForce }) else { return 0.0 }
        let windRatio = (windForce - wind[j - 1]) / (wind[j] - wind[j - 1])

        // Find angle range
        guard let i = gradus.firstIndex(where: { $0 > finalWindAngle }) else { return 0.0 }
    ...
    private func interpolate(windIndex j: Int, angleIndex i: Int, windRatio: Double, angleRatio: Double) -> Double {
        let lowerLeft = diagram[i - 1][j - 1]
        let lowerRight = diagram[i - 1][j]
        let upperLeft = diagram[i][j - 1]
        let upperRight = diagram[i][j]

        let windInterp = (1 - windRatio) * lowerLeft + windRatio * lowerRight
        let angleInterp = (1 - angleRatio) * upperLeft + angleRatio * upperRight

        return (windInterp + angleInterp) / 2.0
    }
}
