...
struct NextTackDistanceView: View {
        ...
        GeometryReader{ geometry in
            ...
            ZStack{
                ...
                case .nauticalMiles:
                    ...
                    Text(String(format: "%.f", (navigationReadings.distanceToNextTack ?? 0) * toNauticalMiles))
                ...
                case .nauticalCables:
                    ...
                    Text(String(format: "%.f", (navigationReadings.distanceToNextTack ?? 0) * toNauticalCables))
                        .frame(width: width, height: width, alignment: .center)
                        .font(Font.custom("Futura-CondensedExtraBold", size: width * 0.4))
                case .meters:
                    Text("mtrs")
                    ...
                    Text(String(format: "%.f", navigationReadings.distanceToNextTack ?? 0))
                        .frame(width: width, height: width, alignment: .center)
                        .font(Font.custom("Futura-CondensedExtraBold", size: width * 0.4))
                ...
                case .boatLength:
                    ...
                    Text(String(format: "%.f", (navigationReadings.distanceToNextTack ?? 0) * toBoatLengths))
                        .frame(width: width, height: width, alignment: .center)
                        .font(Font.custom("Futura-CondensedExtraBold", size: width * 0.4))
...
#Preview {
    NextTackDistanceView(lastUsedUnit: NextTackNauticalDistance())
        .environment(NMEAReader())
        .modelContainer(for: NextTackNauticalDistance.self)
}
